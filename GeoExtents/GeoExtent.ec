...
public Radians wrapLon(Radians x)
{
   if(x < -Pi - radEpsilon)
...
public Radians wrapLonAt(int q, Radians lon, Radians cLon)
{
   lon -= cLon;
...
public struct GeoPoint3D : GeoPoint
{
   Meters alt;

   property GeoPoint
   {
      set { lat = value.lat, lon = value.lon, alt = 0; }
      get { value.lat = lat, value.lon = lon; }
   }

   int OnCompare(GeoPoint3D b)
   {
      if(lat < b.lat) return -1;
      if(lat > b.lat) return  1;
      if(lon < b.lon) return -1;
      if(lon > b.lon) return  1;
      if(alt < b.alt) return -1;
      if(alt > b.alt) return  1;
      return 0;
   }
...
public struct GeoExtent
{
   ...
   property bool nonNull { get { return ll.lat - 1E200 < ur.lat; } }
   ...
   property GeoPoint midPointDL
   {
      get
      {
         value.lat = (ll.lat + ur.lat) / 2;
         if(ur.lon >= ll.lon - radEpsilon)
            value.lon = (ll.lon + ur.lon) / 2;
         else
         {
            value.lon = (Radians)ll.lon + ((Radians)ur.lon + 2*Pi - (Radians)ll.lon) / 2; // TOFIX: Units!!
            if(value.lon > Pi) value.lon -= 2* Pi;
         }
      }
   }
   ...
   {
      // REVIEW: intersectsOrTouchesApprox()
