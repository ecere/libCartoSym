...
public class CartoSymNode : Container
{
   // We don't want to compare contents of these nodes
   class_no_expansion;
   int OnCompare(CartoSymNode b)
   {
      if(this < b) return -1;
      if(this > b) return  1;
      return 0;
...
public:
   virtual void print(File out, int indent, CartoSymOutputOptions o);
   virtual void * /*thisclass FIXME */ copy() { return null; }
   String toString(CartoSymOutputOptions o)
   {
      TempFile f { };
      String s;
      print(f, 0, o);
...
public class CartoSymList : CartoSymNode
{
public:
   List<CartoSymNode> list { };
   ...
   bool SetData(IteratorPointer pointer, D data)          { return list ? list.SetData(pointer, (CartoSymNode)data) : 0; }
...
public:
   ...
   Container ::parse(subclass(Container) c, CartoSymLexer lexer, CartoSymNode parser(CartoSymLexer lexer), char sep)
   {
      Container<CartoSymNode> list = null;
      while(true)
      {
         CartoSymNode e = parser(lexer);
         if(e)
         {
